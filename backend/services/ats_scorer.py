...
def validate_skills_with_projects(
    ...
    # 1. Pre-encode all skills in a single batch
    skill_embeddings = embedder.encode(skills, convert_to_tensor=False)
    
    # 2. Pre-encode all projects in a single batch
    project_texts = [
    ...
    project_embeddings = []
    if project_texts:
        project_embeddings = embedder.encode(project_texts, convert_to_tensor=False)

    # 3. Pre-encode the experience section
    ...
    if experience_text:
        experience_embedding = embedder.encode(experience_text, convert_to_tensor=False)

    validated_skills      = []
    unvalidated_skills    = []
    skill_project_mapping = {}

    # Faster similarity calculator using precomputed vectors
    def _get_similarity(vec1, vec2) -> float:
    ...
    for idx, skill in enumerate(skills):
        skill_emb = skill_embeddings[idx]
        matching_projects = []
        max_similarity    = 0.0

        for p_idx, project in enumerate(projects):
            project_text = project_texts[p_idx]
            ...
            if skill.lower() in project_text.lower():
                ...
                sim = _get_similarity(skill_emb, project_embeddings[p_idx])
        ...
        if experience_text:
            ...
            if skill.lower() in experience_text.lower():
                ...
                sim = _get_similarity(skill_emb, experience_embedding)
                matched = (sim >= threshold)
